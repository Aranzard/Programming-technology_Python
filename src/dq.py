...
def check_non_empty(df: pd.DataFrame, min_rows: int = 1, level: str = "FAIL") -> dict:
    ...
    if len(df) < min_rows:
        result["status"] = "FAIL"
        result["details"] = (
            f"Таблица содержит {len(df)} записей, ожидалось минимум {min_rows}"
        )
    else:
...
def check_unique_key(
    df: pd.DataFrame, key_columns: List[str], level: str = "FAIL"
) -> dict:
    ...
    if len(duplicates) > 0:
        ...
        result["details"] = (
            f"Найдено {len(duplicates)} дублирующихся комбинаций. Примеры: {sample}"
        )
    else:
...
def check_numeric_range(
    ...
    if len(invalid) > 0:
        ...
        result["details"] = (
            f"Все значения в диапазоне: min={df[column].min():.1f}, max={df[column].max():.1f}"
        )
...
def check_allowed_values(
    df: pd.DataFrame, column: str, allowed_values: List, level: str = "FAIL"
) -> dict:
...
def check_freshness(
    ...
    if age_days > max_age_days:
        result["status"] = level
        result["details"] = (
            f"Последние данные от {max_date.date()}, прошло {age_days} дней (максимум {max_age_days})"
        )
    else:
        result["details"] = (
            f"Данные актуальны. Последняя запись: {max_date.date()}, прошло {age_days} дней"
        )
...
def check_normalized(df: pd.DataFrame) -> List[dict]:
    ...
    results.append(
        check_numeric_range(df, "mag", min_value=0, max_value=10, level="WARNING")
    )
...
def run_all_dq(
    data_dir: Path, layers: List[str] = ["normalized", "mart"]
) -> List[dict]:
...
def main():
    data_dir = Path(__file__).parent.parent / "docs/dq"
